...
usr = WorkPageUser()
response_login = usr.login(username=name,password=password,autosign=False)
...
if response_login.status_code == 200:#登录成功
    ...
    while True:
        ...
        new_works_data = requests.get(url=url_get_list,headers=headers).json()
        ...
        for newwork_count in new_works_list:
            try:
                ...
                """
                #查看作品本身是否已点赞
                is_like = usr.work_has_liked(work_id)
                #点过赞就不点了
                if is_like:
                ...
                while not is_like:
                    response_like = usr.work_like(work_id)
                    is_like = usr.work_has_liked(work_id)#查看是否已点赞
                    if not is_like:
                        print(f"编号为{work_id}的作品点赞失败,60s后重试")
                        sleep(60)
                ...
                content = random.choice(comment_list).replace("{}",("@"+nickname))#加上昵称处理
                # what = what.format(("@"+nickname)) if "{}" in what else what#加上昵称处理
                #评论
                if int(user_id) in baimingdan:
                    print(f"编号为 {work_id} 的作者在白名单内，不予评论")
                    continue
                response_comment = usr.work_comment(work_id=work_id,content=content)
                ...
                print(f"id为{work_id}的作品捕获到异常{e}")
